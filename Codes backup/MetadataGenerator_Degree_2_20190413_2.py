...
def get_country_degree_by_name(list,countryName):
    return [x for x in list if x[0]==countryName][0][1];
...
def sorted_by_degree_based_on_most_recent_year(inputlist):
    ...
    sorted_data = {y: [(c, get_country_degree_by_name(inputlist[y],c)) for c in max_year_countries] for y in years if y!=max_year}  #####check this part####
...
def plot(list_of_splitted_data, name): #{year:[(countryName,degree)]}
    ...
    width = 0.5
    fig,ax=plt.subplots()
    fig.set_size_inches(10.14, 10.14)
